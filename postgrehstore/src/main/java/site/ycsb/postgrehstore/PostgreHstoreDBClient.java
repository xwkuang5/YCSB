package site.ycsb.postgrehstore;
...
public class PostgreHstoreDBClient extends DB {

  private static final Logger LOG = LoggerFactory.getLogger(PostgreHstoreDBClient.class);
   ...
   */
  public static final String CONNECTION_URL = "postgrehstore.url";

  /**
   * The user name to use to connect to the database.
   */
  public static final String CONNECTION_USER = "postgrehstore.user";

  /**
   * The password to use for establishing the connection.
   */
  public static final String CONNECTION_PASSWD = "postgrehstore.passwd";

  /**
   * The JDBC connection auto-commit property for the driver.
   */
  public static final String JDBC_AUTO_COMMIT = "postgrehstore.autocommit";

  /**
   * The transaction isolation level.
   */
  public static final String ISOLATION_LEVEL = "postgrehstore.isolationlevel";
  ...
  public Status update(String tableName, String key, Map<String, ByteIterator> values) {
    try {
      ...
      Map<String, String> hstoreMap = values.entrySet().stream()
          .collect(Collectors.toMap(Entry::getKey, e -> e.getValue().toString()));
      updateStatement.setObject(1, hstoreMap);
  ...
  public Status insert(String tableName, String key, Map<String, ByteIterator> values) {
    try {
      ...
      insertStatement.setObject(2, hstoreMap);
  ...
  private String createReadStatement(StatementType readType) {
    ...
    if (readType.getFields() == null) {
      read.append(", (each(" + COLUMN_NAME + ")).*");
    } else {
      for (String field : readType.getFields()) {
        read.append(", " + COLUMN_NAME + "::hstore -> '" + field + "' AS " + field);
      }
    }
  ...
  private String createScanStatement(StatementType scanType) {
    ...
    if (scanType.getFields() != null) {
      for (String field : scanType.getFields()) {
        scan.append(", " + COLUMN_NAME + "::hstore -> '" + field + "' AS " + field);
      }
    }
